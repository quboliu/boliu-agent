...
`dual-paragraph-gap = paragraph-gap` is the only body gap in the bilingual entry
module. It applies equally between the original and translation and between
adjacent paired units; `dual` has no independent language-gap or pair-gap
arguments. If the shared body rhythm needs a reviewed project-wide override,
change the copied core's `paragraph-gap` token once. Do not add per-paragraph
manual spacers or reintroduce separate bilingual gap values. The regression
suite measures exported PDF lines and rejects a second, larger pair gap.
